...
void AH_EasyDriver::setSpeedRPM(double RPM)
{
  long DIV = (double) RPM*this->MOTOR_RESOLUTION*this->STEPPING_FACTOR;
  this->STEP_DELAY = 60L*1000000L/DIV;
...
void AH_EasyDriver::revolve(float TIMES, boolean DIRECTION){

  int steps_to_move = (int)(TIMES*(this->MOTOR_RESOLUTION)*(this->STEPPING_FACTOR));
  move(steps_to_move, DIRECTION);
...
void AH_EasyDriver::move(int NUMBER_OF_STEPS)
{
  ...
  while(STEPS > 0) {
   if (((double)micros() - this->LAST_STEP_TIME) >= this->STEP_DELAY) {
      this->LAST_STEP_TIME = (double)micros();
      STEPS--;
      stepMotor();
    }
  }
...
void AH_EasyDriver::stepMotor(boolean DIR)
{
	setDirection(DIR);
